...
export class ContactController {
  ...
  async sendContact(@Body() body: ContactDto, @Req() req: Request) {
    ...
    try {
      ...
      await this.contactWebhookService.saveAndNotify(
        body.name,
        body.email,
        body.message,
      );
      this.logger.log(
        `Kontaktní formulář úspěšně odeslán uživatelem ${body.email}`,
      );
      ...
      this.logger.error(
        `Nepodařilo se zpracovat kontaktní formulář: ${error.message}`,
      );
      throw new HttpException(
  ...
  async getSubmissions() {
    try {
      ...
      throw new HttpException(
        'Nepodařilo se načíst odeslané zprávy',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async markAsRead(@Param('id') id: string) {
    try {
      ...
      throw new HttpException(
        'Nepodařilo se aktualizovat stav zprávy',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
