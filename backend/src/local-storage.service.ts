// Legacy file-system image storage kept only for backwards-compatible cleanup.
...
export class LocalStorageService {
  static isLegacyLocalFileUrl(fileUrl: string): boolean {
  ...
  static async deleteFile(fileUrl: string): Promise<void> {
    try {
      if (!this.isLegacyLocalFileUrl(fileUrl)) {
        return;
      }
